...
from application.forms import CreatePlayerForm
from flask import render_template, request, redirect, url_for, jsonify
import requests

backend_host = "footy_backend:5000"
@app.route('/', methods+["GET"])
def home():
    players = request.get(f"http://{backend}/get/allPlayers").json()
 ...
 def create_player():
     ...
     if request.method == "POST":
        response = requests.post(
             f"http://{backend_host}/create/player",
            json={
                "name": form.name.data
